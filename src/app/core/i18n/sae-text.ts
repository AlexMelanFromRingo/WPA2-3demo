  ...
  steps: [
    ...
    },
    {
      ...
      terms: [
        ...
        { term: 'expand_message_xmd', definition: 'этап hash_to_curve: пароль и служебные строки многократно прогоняют через SHA-256, «растягивая» в длинную строку байт нужной длины.' },
        { term: 'Поле, модуль p', definition: 'все числа кривой берут по модулю p — большого простого числа кривой P-256. «По модулю p» = взять остаток от деления на p.' },
        { term: 'Сложение точек', definition: 'операция на эллиптической кривой: из двух точек по геометрическому правилу получается третья. Так складывают Q0 и Q1.' },
      ],
      ...
      calc: [
        ...
        '  SSID   = {ssidHex}',
        '',
        'hash_to_curve (RFC 9380) — это НЕ одна операция, а 4 этапа.',
        'Задача: честно превратить пароль в точку (x, y) на кривой',
        'P-256 — без «подбора» и за постоянное время.',
        '',
        '1) expand_message_xmd',
        '   Пароль и служебные строки прогоняют через SHA-256 и',
        '   «растягивают» в длинную строку байт.',
        '',
        '2) hash_to_field',
        '   Строку делят на две части; каждую берут по модулю p',
        '   (большое простое кривой P-256) → два числа поля u0 и u1.',
        '',
        '3) map_to_curve (SSWU)',
        '   Каждое число u по фиксированной формуле Simplified SWU',
        '   превращают в точку кривой — без перебора. Выходят Q0 и Q1.',
        '',
        '4) Сложение точек',
        '   Q0 + Q1 по правилу сложения на кривой → точка PWE.',
        '   (Очистка кофактора: у P-256 кофактор = 1, шаг пустой.)',
        '',
  ...
  steps: [
    ...
    },
    {
      ...
      terms: [
        ...
        { term: 'expand_message_xmd', definition: 'етап hash_to_curve: пароль і службові рядки багаторазово проганяють через SHA-256, «розтягуючи» у довгий рядок байтів потрібної довжини.' },
        { term: 'Поле, модуль p', definition: 'усі числа кривої беруть за модулем p — великого простого числа кривої P-256. «За модулем p» = взяти остачу від ділення на p.' },
        { term: 'Додавання точок', definition: 'операція на еліптичній кривій: із двох точок за геометричним правилом виходить третя. Так додають Q0 і Q1.' },
      ],
      transform: 'hash-to-curve на P-256 (SSWU)',
      calc: [
        'ВХІД:',
        ...
        '  SSID   = {ssidHex}',
        '',
        'hash_to_curve (RFC 9380) — це НЕ одна операція, а 4 етапи.',
        'Завдання: чесно перетворити пароль на точку (x, y) на кривій',
        'P-256 — без «підбору» і за сталий час.',
        '',
        '1) expand_message_xmd',
        '   Пароль і службові рядки проганяють через SHA-256 і',
        '   «розтягують» у довгий рядок байтів.',
        '',
        '2) hash_to_field',
        '   Рядок ділять на дві частини; кожну беруть за модулем p',
        '   (велике просте кривої P-256) → два числа поля u0 і u1.',
        '',
        '3) map_to_curve (SSWU)',
        '   Кожне число u за фіксованою формулою Simplified SWU',
        '   перетворюють на точку кривої — без перебору. Виходять Q0 і Q1.',
        '',
        '4) Додавання точок',
        '   Q0 + Q1 за правилом додавання на кривій → точка PWE.',
        '   (Очищення кофактора: у P-256 кофактор = 1, крок порожній.)',
        '',
  ...
  steps: [
    ...
    },
    {
      ...
      terms: [
        ...
        { term: 'expand_message_xmd', definition: 'a stage of hash_to_curve: the password and helper strings are run through SHA-256 repeatedly and “expanded” into a long byte string of the required length.' },
        { term: 'Field, modulus p', definition: 'all curve numbers are taken modulo p — the large prime of curve P-256. “Modulo p” = take the remainder of division by p.' },
        { term: 'Point addition', definition: 'an elliptic-curve operation: from two points a third is obtained by a geometric rule. This is how Q0 and Q1 are added.' },
      ],
      ...
      calc: [
        ...
        '  SSID     = {ssidHex}',
        '',
        'hash_to_curve (RFC 9380) is not a single operation but 4 stages.',
        'The task: honestly turn the password into a point (x, y) on',
        'curve P-256 — with no “searching” and in constant time.',
        '',
        '1) expand_message_xmd',
        '   The password and helper strings are run through SHA-256 and',
        '   “expanded” into a long byte string.',
        '',
        '2) hash_to_field',
        '   The string is split in two; each half is taken modulo p',
        '   (the large prime of curve P-256) → two field numbers u0 and u1.',
        '',
        '3) map_to_curve (SSWU)',
        '   Each number u is turned into a curve point by the fixed',
        '   Simplified SWU formula — no searching. This yields Q0 and Q1.',
        '',
        '4) Point addition',
        '   Q0 + Q1 by the curve addition rule → the point PWE.',
        '   (Cofactor clearing: for P-256 the cofactor is 1 — a no-op.)',
        '',
